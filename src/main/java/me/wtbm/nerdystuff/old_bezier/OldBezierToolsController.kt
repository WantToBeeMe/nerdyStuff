...
object OldBezierToolsController {
    ...
    fun toolsTick(){
        Bukkit.getOnlinePlayers().forEach(){p->
            ...
            if(meta.customModelData == OldBezierCurveController.createId){
                ...
                OldBezierCurveController.bezierCurves[name]?.generateLocListWithExtra(p.location)
            }
        }
    }
    ...
    fun reworkItemMake(p: Player){
        ...
        itemMeta.setDisplayName("${ChatColor.LIGHT_PURPLE}${ChatColor.BOLD}Create ${ChatColor.RESET}${ChatColor.GRAY}(${OldBezierCurveController.bezierCurves[name]?.getPivotAmount()})")
    ...
    fun reworkItemUndo(p: Player){
        ...
        val itemMeta = item?.itemMeta
        val name : String = itemMeta?.lore!![0].split('[',']')[1]
        itemMeta.setDisplayName("${ChatColor.RED}${ChatColor.BOLD}Undo ${ChatColor.RESET}${ChatColor.GRAY}(${OldBezierCurveController.bezierCurves[name]?.getPivotAmount()})")
        item.itemMeta = itemMeta
        p.inventory.setItemInMainHand(item)
    }
    ...
    fun giveTools(p: Player, name:String){
        ...
        createMeta?.setDisplayName("${ChatColor.LIGHT_PURPLE}${ChatColor.BOLD}Create ${ChatColor.RESET}${ChatColor.GRAY}(${OldBezierCurveController.bezierCurves[name]?.getPivotAmount()})")
        createMeta?.lore = (listOf("${ChatColor.DARK_GRAY}For creating new pivots for${ChatColor.GRAY} [$name]"))
        createMeta?.setCustomModelData(OldBezierCurveController.createId)
        ...
        moveMeta?.setCustomModelData(OldBezierCurveController.moveId)
        ...
        undoMeta?.setDisplayName("${ChatColor.RED}${ChatColor.BOLD}Undo ${ChatColor.RESET}${ChatColor.GRAY}(${OldBezierCurveController.bezierCurves[name]?.getPivotAmount()})")
        undoMeta?.lore = (listOf("${ChatColor.DARK_GRAY}For undoing the last pivots for${ChatColor.GRAY} [$name]"))
        undoMeta?.setCustomModelData(OldBezierCurveController.undoId)
